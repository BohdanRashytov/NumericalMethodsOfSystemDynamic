  ...
  val function: Map[Int, Map[(Int, Int), Double]] = {
    ...
    t.foreach(t => if (t != 0) {
      ...
      (1 to N - 1).foreach { m =>
        ...
        (1 to N).foreach(i => betaI = betaI.+(i -> -(FI(i) - A * betaI(i - 1)) / (A * alphaI(i - 1) + C)))
        halfTimeMap = halfTimeMap.+((N, m) -> ((FI(N) - A * betaI(N)) / (C + A * alphaI(N))))
        (1 to N - 1).map(i => N - i).foreach(k =>
          halfTimeMap = halfTimeMap.+((k, m) -> (alphaI(k) * halfTimeMap(k + 1, m) + betaI(k)))
        )
      }
      ...
      (1 to N - 1).foreach { k =>
        ...
        (1 to N - 1).map(i => N - i).foreach(m =>
          tempMap = tempMap.+((k, m) -> (alphaI(m) * tempMap(k, m + 1) + betaI(m)))
        )
      }
